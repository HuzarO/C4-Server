        ...
        if (!_actor.isMuted() && dist_2 > (range + 20) * (range + 20))
        {
            ...
            if (!Config.ALT_GAME_MOB_ATTACK_AI
                ...
                for (L2Skill sk : skills)
                {
                    ...
                    if (((sk.getSkillType() == L2Skill.SkillType.BUFF || sk.getSkillType() == L2Skill.SkillType.HEAL) || (dist_2 >= (castRange / 3) * (castRange / 3))
            ...
            else
            {
                ...
                if ((dx * dx + dy * dy > 100 * 100) && (homeX * homeX + homeY * homeY > 1200 * 1200)
                    && (_actor.getKnownList().knowsObject(_attack_target)))
                {
                    // Cancel the target
                    _actor.getKnownList().removeKnownObject(_attack_target);
